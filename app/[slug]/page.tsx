...
function makeCoreWatchSalesSchema(slug: "brands-we-carry" | "buy-premium-watches-at-special-price" | "sell-your-luxury-watch") {
  const configurations = {
    "brands-we-carry": { name: "Watch Brands We Carry in Johns Creek, GA", description: "Explore authentic Seiko, Citizen, Bulova, Luminox, Hamilton, and G-SHOCK watches at It’s About Time in Johns Creek with free wrist sizing and manufacturer warranty coverage.", image: "/assets/pages/brands-we-carry-seiko-logo.webp", type: "store", faqs: [] },
    "buy-premium-watches-at-special-price": { name: "Buy Premium Watches in Johns Creek, GA", description: "Explore premium watch collections in person at It’s About Time in Johns Creek, serving Atlanta-area watch owners since 1979.", image: "/assets/pages/premium-watches-special-price-hero.jpg", type: "store", faqs: [] },
    "sell-your-luxury-watch": { name: "Sell Your Luxury Watch in Johns Creek, GA", description: "Submit luxury watch details for a no-obligation assessment by certified watchmakers at It’s About Time in Johns Creek.", image: "/assets/pages/sell-your-luxury-watch-hero.jpg", type: "valuation", faqs: [["Do I need to have the watch serviced before selling?", "No. Watches in any condition, including those needing service, can be submitted. Condition is factored into the offer."], ["Who covers shipping costs?", "Shipping to the Johns Creek workshop is at your expense. If an agreement is not reached, return shipping is also your responsibility."], ["What happens if we cannot agree on a price?", "There is no obligation to accept an offer. If you decline, the watch is shipped back promptly."], ["How do I know my watch is safe while it is with you?", "Your watch stays in the Johns Creek workshop from arrival to shipment and is handled in-house by certified watchmakers."], ["Can I sell a watch without its original box or papers?", "Yes. Box and papers are not required, though their presence can affect the offer."], ["How long does the process take?", "The team follows up on submissions within 1–5 business days and aims to present an offer within 1–5 business days after receiving the watch."]] }
  } as const;
  const config = configurations[slug];
  const pageUrl = siteUrl + "/" + slug + "/";
  const graph: Record<string, unknown>[] = [
...
function makeFinalRepairWorkshopSchema(slug: "expert-rolex-watch-repair-buford" | "watch-battery-replacement-in-suwanee-ga") {
  const configurations = {
    ...
    "@graph": [
      {
        ...
        "@id": `${pageUrl}#webpage`,
        url: pageUrl,
        name: "About It’s About Time | Johns Creek Watchmakers",
        description: "Learn about It’s About Time Inc., a Johns Creek watch repair, service, and sales business built on craftsmanship, transparency, and respect for fine timepieces.",
        primaryImageOfPage: `${siteUrl}/assets/pages/atlanta-watch-service-center-workshop-duluth-hero.jpg`,
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const localRepairMetadata = {
    ...
    "brands-we-carry": { title: "Watch Brands We Carry in Johns Creek, GA | It’s About Time", description: "Explore authentic Seiko, Citizen, Bulova, Luminox, Hamilton, and G-SHOCK watches in Johns Creek with free wrist sizing and original manufacturer warranty coverage.", keywords: ["watch brands Johns Creek", "authorized watch dealer", "Seiko Citizen Bulova watches", "watch showroom Georgia"], image: "/assets/pages/brands-we-carry-seiko-logo.webp", imageWidth: 600, imageHeight: 300, alt: "Seiko logo from the legacy Brands We Carry page" },
    "buy-premium-watches-at-special-price": { title: "Buy Premium Watches in Johns Creek, GA | It’s About Time", description: "Explore premium watches in Johns Creek. Discover a curated showroom selection, brand comparisons, and personal guidance from a trusted local watch seller since 1979.", keywords: ["buy premium watches Johns Creek", "watch store Atlanta", "luxury watches Georgia", "premium watch showroom"], image: "/assets/pages/premium-watches-special-price-hero.jpg", imageWidth: 1024, imageHeight: 682, alt: "Monochrome wristwatch from the archived premium-watch source page" },
    "sell-your-luxury-watch": { title: "Sell Your Luxury Watch in Johns Creek, GA | It’s About Time", description: "Sell a luxury watch through an informed, no-obligation assessment from certified watchmakers in Johns Creek. Submit watch details, photos, and condition information to get started.", keywords: ["sell luxury watch Johns Creek", "sell Rolex Atlanta", "watch valuation Georgia", "luxury watch buyer"], image: "/assets/pages/sell-your-luxury-watch-hero.jpg", imageWidth: 2048, imageHeight: 1367, alt: "Watchmaker inspecting a luxury watch from the legacy sale page" }
  } as const;
...
export default async function ConvertedPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      {(slug === "brands-we-carry" || slug === "buy-premium-watches-at-special-price" || slug === "sell-your-luxury-watch") && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(makeCoreWatchSalesSchema(slug)) }} />}
